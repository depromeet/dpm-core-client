...
const _BillDetailContainer = ({ billId }: { billId: number }) => {
	...
	return (
		<>
			...
			<BillFooterAction billStatus={bill.billStatus} billId={billId} />
		</>
	);
};
...
const BillFooterAction = ({ billStatus, billId }: { billStatus: BillStatus; billId: number }) => {
	const { mutate } = useMutation(closeBillParticipationMutationOptions());
	...
	return (
		<footer>
			<Button size="full" variant="secondary" onClick={handleCloseBillParticipation}>
