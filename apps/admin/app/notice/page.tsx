...
const NoticePage = () => {
  ...
  const filteredNotices = notices.filter((notice) => {
    ...
    if (selectedType === 'assignment')
      return notice.tags.includes('assignment');
  ...
  return (
    <SidebarInset>
      <div className="flex h-screen max-w-full flex-col items-center">
        ...
        {/* Body */}
        <div className="flex flex-1 w-full flex-col items-start bg-background-normal">
          {/* Title + Button */}
          <div className="flex w-full items-center justify-between px-10 pt-6">
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-label-normal text-title1">
                공지 목록
              </h2>
          ...
          {filteredNotices.length === 0 && (
            <div className="flex min-h-100 w-full items-center justify-center">
              <p className="text-label-alternative text-body1">
                등록된 공지가 없습니다.
              </p>
            </div>
          )}
