...
interface SettleListItemProps {
	bill: Bill;
}

const SettleListItem = ({ bill }: SettleListItemProps) => {
	const { title, billStatus, createdAt, answerMemberCount, inviteGroups, gatherings } = bill;
	const formattedDate = dayjs(createdAt).format('YY년 M월 D일 (dd)');
	const roundCount = gatherings.length;
		...
		case 'COMPLETED':
			return (
				<>
					...
					<p className="text-caption1 text-gray-400 font-semibold">정산 끝</p>
				</>
			);
		default:
			return null;
