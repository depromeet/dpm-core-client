...
interface StaffDinnerEditBottomSheetProps {
...
export const StaffDinnerEditBottomSheet = ({
	attendance,
	onEditSuccess,
	children,
	isClosed = false,
	gatheringId,
}: StaffDinnerEditBottomSheetProps) => {
	const [isOpen, setIsOpen] = useState(false);
	const [selectedAttendance, setSelectedAttendance] = useState<AttendanceStatus>(attendance);
	const { ref } = useAppShell();
	const queryClient = useQueryClient();

	const { mutate, isPending } = useMutation(submitRsvpStatusMutationOptions(gatheringId));
	...
	const handleSubmit = () => {
		...
		mutate(
			{ isRsvpGoing: toRsvpGoing(selectedAttendance) },
			{
				onSuccess: () => {
