  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 px-4 pb-4 pt-8">
        <h3 className="flex-1 font-semibold text-title2 text-label-normal">
          {title}
        </h3>
      ...
      <div className="flex flex-col gap-6 px-8 py-6">
        {sections.map(({ label, options }) => (
          <ChipGroup key={label} label={label}>
            {options.map((option) => {
              const isSelected =
                selectedValues[label]?.includes(option.value) || false;
      ...
      {/* Footer */}
      <div className="flex gap-2 bg-background-normal px-8 pb-8 pt-6">
