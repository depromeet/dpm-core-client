...
export const MultiActionToolbar = ({
  ...
  return (
    <div
      className={cn(
        'flex h-17.5 items-center gap-4 border-t border-line-subtle bg-background-normal py-3',
        className,
      )}
      {...props}
    >
      <p className="flex-1 font-medium text-body1 text-label-subtle">
        {active ? (
          <>
            <span className="text-primary-normal">{selectedCount}명</span>{' '}
            선택됨
          </>
        ) : (
          `전체 ${totalCount}명`
      ...
      <div className="flex items-center gap-2">
        {actions.map(({ onClick, disabled, label }, index) => (
          <Button
            key={index}
            variant="secondary"
            size="md"
